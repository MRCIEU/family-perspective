...
Now convert to context of betas rather than covarainces?
...
params_x <- sim2(10000, 0.4, 0.2, 5000, 0.8, -1, 0.6)
d <- params_x %>%
  arrange(rsq_x) %>%
...
params <- expand.grid(
  ...
  S_2 = 0, 
  rho = c(0.18),
  n = seq(1000000,100000000,by=1000000)
)

d_power <- lapply(1:nrow(params), function(i)
{
  params_x <- do.call(sim2, params[i,] %>% dplyr::select(-n)) %>%
    mutate(power=pwr.r.test(n=params$n[i], r=sqrt(rsq_x), sig.level=5e-8)$power, n=params$n[i]) %>%
    group_by(what) %>% summarise(nsig=sum(power)) %>% 
    bind_cols(., params[i,])
}) %>% bind_rows()
